...
let api = {
  ...
  http(_options, cb, errCb, allData) {
    let urlName = _options.url;
    _options.url = apis[urlName].url;
    if (_options.data) {
      for (let k in _options.data) {
        if (!_options.data[k] && _options.data[k] !== 0) {
          delete _options.data[k];
        }
      }
    }
    if (_options.params) {
      for (let k in _options.params) {
        if (!_options.params[k] && _options.params[k] !== 0) {
          delete _options.params[k];
        }
      }
    }
    try {
